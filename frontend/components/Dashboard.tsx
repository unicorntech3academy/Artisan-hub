...
export const Dashboard: React.FC = () => {
  const { currentUser, jobs, profiles } = useApp();
  const [isModalOpen, setIsModalOpen] = useState(false);
  const [selectedJob, setSelectedJob] = useState<Job | null>(null);
  const [isBidModalOpen, setIsBidModalOpen] = useState(false);

  const filteredJobs = currentUser?.role === UserRole.OWNER 
    ? jobs.filter((j: Job) => j.ownerId === currentUser.id)
    : jobs.filter((j: Job) => j.status === JobStatus.OPEN);
  ...
  const stats = currentUser?.role === UserRole.OWNER ? [
    ...
    { label: 'Completed', value: filteredJobs.filter((j: Job) => j.status === JobStatus.COMPLETED).length, icon: CheckCircle, color: 'text-emerald-600', bg: 'bg-emerald-50' },
    { label: 'In Progress', value: filteredJobs.filter((j: Job) => j.status === JobStatus.IN_PROGRESS).length, icon: Clock, color: 'text-amber-600', bg: 'bg-amber-50' },
  ] : [
    { label: 'Available Jobs', value: jobs.filter((j: Job) => j.status === JobStatus.OPEN).length, icon: Briefcase, color: 'text-blue-600', bg: 'bg-blue-50' },
